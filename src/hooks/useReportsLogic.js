...
export const useReportsLogic = () => {
  ...
  const handleClearData = () => {
    Alert.alert("Reset", "Delete all history?", [
      { text: "Cancel" },
      { text: "Delete", style: "destructive", onPress: async () => { await clearData(); await fetchData(); Alert.alert("Cleared", "All session data removed."); } }
    ]);
  };

  const handleSeedDemo = () => {
    Alert.alert("Load demo data?", "This adds sample sessions for the last 7 days.", [
      { text: "Cancel" },
      { text: "Seed", onPress: async () => { await seedDatabase(); fetchData(); } }
    ]);
  };
  ...
  return { viewMode, setViewMode, stats, pieData, barData, barLabels, recentSessions, handleClearData, handleSeedDemo };
};